  ...
  if (variant === 'table') {
    return (
      <div className={`space-y-2 ${className}`} aria-busy="true" aria-label="Loading data">
        <ShimmerBox className="h-8 w-full opacity-50" />
    ...
    return (
      <div className={`grid grid-cols-2 gap-4 ${className}`} aria-busy="true">
        {Array.from({ length: count }).map((_, i) => (
          <ShimmerBox key={i} className="h-40 w-full rounded-lg" />
        ))}
      </div>
    );
  }
  if (variant === 'chart') {
    return <ShimmerBox className={`h-64 w-full rounded-lg ${className}`} aria-busy={true} />;
  }
  if (variant === 'kpi') {
    return (
      <div className={`grid grid-cols-2 lg:grid-cols-${count} gap-4 ${className}`} aria-busy="true">
        {Array.from({ length: count }).map((_, i) => (
          <ShimmerBox key={i} className="h-28 w-full rounded-lg" />
        ))}
      </div>
    );
  }
