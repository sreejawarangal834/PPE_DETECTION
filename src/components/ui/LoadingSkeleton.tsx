...
function ShimmerBox({ className = '' }: { className?: string }) {
  return <div className={`bg-panel-alt/50 animate-pulse rounded ${className}`} />;
}
  ...
  if (variant === 'table') {
    return (
      <div className={`space-y-2 ${className}`} aria-busy="true" aria-label="Loading data">
        <ShimmerBox className="h-12 w-full bg-panel-alt" />
    ...
    return (
      <div className={`grid grid-cols-2 gap-4 ${className}`} aria-busy="true">
        {Array.from({ length: count }).map((_, i) => (
          <ShimmerBox key={i} className="h-40 w-full rounded-xl bg-panel/50" />
        ))}
      </div>
    );
  }
  if (variant === 'chart') {
    return <ShimmerBox className={`h-64 w-full rounded-xl bg-panel/50 ${className}`} aria-busy={true} />;
  }
  if (variant === 'kpi') {
    return (
      <div className={`grid grid-cols-2 lg:grid-cols-${count} gap-4 ${className}`} aria-busy="true">
        {Array.from({ length: count }).map((_, i) => (
          <ShimmerBox key={i} className="h-28 w-full rounded-xl bg-panel/50" />
        ))}
      </div>
    );
  }
