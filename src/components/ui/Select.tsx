...
export default function Select({ label, options, error, placeholder, className = '', id, ...rest }: SelectProps) {
  ...
  return (
    <div className={`flex flex-col gap-1 ${className}`}>
      {label && <label htmlFor={selectId} className="text-sm text-text-secondary font-medium">{label}</label>}
      <select
        {...rest}
        id={selectId}
        aria-invalid={!!error}
        className={`
          w-full bg-panel-alt border rounded-md px-3 py-2 text-sm text-text-primary
